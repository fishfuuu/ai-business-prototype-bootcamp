...
状态：Official Execution v1.6 Master Scaffolding Edition（黑盒工程护盾、Excel草稿双签比喻与双Agent决策树版）  
更新时间：2026-08-10  

内容来源与权威说明：
- `docs/COURSE_ROADMAP.md`：**结合认知架构重构、黑盒工程护盾、Excel草稿双签比喻、双Agent CEO决策树与【四步概念公式】整理后的唯一权威执行版（Authoritative Execution Plan）。**
...
主管不需要去搞底层的生产服务器运维、复杂并发代码或手打 Git/Patch 命令行，而是要掌握：
1. **高保真原型控盘能力**：在 2 小时内用自然语言 + AI 跑出可运行、可点击、带模拟数据的高保真原型系统。
2. **需求与数据契约表达能力**：将口头需求标准化为《业务功能卡》、可视化数据契约卡与明确的输入/输出边界。
3. **黑盒工程护盾与版本证据管理能力**：掌握安全沙箱授权、`CLAUDE.md` 护栏、Grounding (事实锚定)、三分记忆模型（工作记忆、外部长期记忆、版本证据）与一键黑盒降级恢复方法（`npm run doctor -- --rollback`），防止项目失控。
4. **可复核证据驱动的验收与审查能力**：基于可复核、可重复生成的四类证据卡片（视觉、行为、工程、范围）完成自动化验收，并在独立审查上下文隔离下依据《CEO 3 大硬性决策树》引导 Codex 进行代码审查与一票裁决。
...
- **第二层：Harness Engineering (工程护栏)** —— 通过规则 (`CLAUDE.md`)、文件 (`PROJECT_STATE.md`)、Skills (`grill-me`, `design-lint`)、黑盒测试脚本 (`verify-project.ps1` / `npm run verify`)、权限与工作流约束 Agent 执行。
...
3. **具象业务比喻 (Business Metaphor)**：建立大脑记忆锚点，降低心理门槛（如：把 Git 双 Commit 比喻为“Excel 表格草稿录入 (Commit A 源码草稿) 与 部门主管签字盖章确权 (Commit B 业务确权)”；把 TS 类型契约比喻为“业务表单必填字段规范卡”）。
...
- **依据《CEO 3 大硬性决策树》** 签署人在回路 (HITL) 授权，完成最终业务验收
...
6. **黑盒排错护盾与 2 轮硬熔断降级 (Bounded Debugging & Automated Rollback Shield)**：
   第 6 课排错严格限定在“界面呈现与业务契约不一致”的领域，严禁主管手打 Git/Patch 命令或深挖底层代码；同一 Bug 最多由 Agent 自修 2 轮，超轮次由黑盒脚本 `npm run doctor -- --rollback` 自动还原稳定源码并打上只读故障标记，阻断死循环。
7. **独立上下文审查与 CEO 3 大硬性决策树 (Context-Isolated Audit & Arbitration Decision Tree)**：
   第 8 课 Codex 必须在全新的只读独立会话中开启，只读取 Candidate Commit 的版本变更；若双 Agent 辩论满 2 轮未达成一致，主管依据《CEO 3 大硬性决策树》（业务优先/安全红线/范围防漂移）行使一票裁决权。
...
| **二、需求与结构** | **3** | 把模糊想法变成可执行的业务契约 | **Prompt vs Skill Harness (`grill-me`)**；前置 **6 大要素**；脱敏规约；黑盒 TS 契约生成与数据契约卡 | **ReAct + Skill 约束**；`grill-me` | 《业务功能卡与数据契约卡》(`BUSINESS_FEATURE_CARD.md`) + 契约状态卡 + Mock 数据 |
| | **4** | 把大需求拆成连续的小成功 | **增量实施 (Plan & Execute)**；持久化计划 (`LESSON_04_IMPLEMENTATION_PLAN.md`)；Excel草稿双签比喻；三层验收与版本归档 | **Plan & Execute 范式**；**后台静默 Verifier Subagent** (跑自测与日志落盘) | 已批准实施计划 + 一次完整版本归档 (Commit A 源码草稿 + Commit B 业务确权) |
| **三、防崩与排错** | **5** | 建立不会轻易失控的项目 | **工程 Harness (`CLAUDE.md`)**；**三分记忆模型**；**【解药 1：环境脱幻与独立工具箱】** | **Plan & Execute 范式**；`CLAUDE.md` 项目护栏 | 带项目护栏、一键黑盒恢复与独立工具箱的稳定系统 |
| | **6** | 学会定位和修复问题 | **事实锚定排错**；五层诊断卡；**契约排错 & 2轮黑盒硬熔断 (`npm run doctor -- --rollback`)** | **ReAct 有界自修**；黑盒诊断卡输出 | 带有五层可视化诊断卡与自动降级标记的系统 |
| **四、双 Agent 验收** | **7** | 让 Agent 实际操作页面完成验收 | **黑盒断言 Harness (`npm run test:ui`)**；停止条件转自动化断言；四类可复核证据卡 | **Subagent + Browser MCP** (黑盒捕获截图与动作) | 带有四类可复核证据卡片 (视觉/行为/工程/范围) 的验收记录 |
| | **8** | Claude Code 开发，Codex 独立审查 | **独立审查上下文隔离 (`AGENTS.md`)**；只读审查；**CEO 3 大硬性决策树** 仲裁门禁 | **Multi-Agent 范式** (开发 Agent + Codex 审计) | 经过双 Agent 审计与主管决策树裁决的代码 |
...
* **主要内容**：
  ...
  4. **三层验收与干净项目工作区恢复**：结合后台 Verifier 静默自测、人工页面点击与主管业务验收完成一次完整版本归档 (Commit A 源码草稿 + Commit B 业务确权)；自测失败时，运行黑盒恢复脚本 `npm run doctor -- --rollback` 自动还原干净源码，仅提交 Commit B 状态并将步骤标记为 `BLOCKED`。
...
### 第 6 课：学会定位和修复问题 — 五层诊断卡与黑盒排错护盾
* **定位**：事实锚定排错、五层可视化诊断卡、契约层排错防跑偏与黑盒 2 轮硬熔断护盾。
* **主要内容**：
  1. **五层诊断卡 (Five-Layer Diagnostic Map)**：按物理环境 ➔ 数据源 ➔ 组件状态 ➔ 日志 ➔ 契约断言五层排查，控制台直接打印自然语言诊断卡。
  2. **排错范围严格限定**：只排“界面呈现与业务契约不一致”的 Bug，严禁深入底层复杂代码。
  3. **2 轮黑盒硬熔断机制**：同一 Bug 最多由 Agent 自修 2 轮；若未 PASS，黑盒脚本 `npm run doctor -- --rollback` 自动还原稳定源码并打上只读故障标记，完全切断主管手打 Git/Patch 命令的操作瓶颈。

---

### 第 7 课：让 Agent 实际操作页面完成验收 — 浏览器 MCP 与四类证据卡
* **定位**：断言 Harness 自动化，Playwright / Browser-Tester Subagent 驱动页面自动化点击并黑盒输出四类可复核证据卡片。
* **主要内容**：
  1. 挂载 Browser MCP，学员运行 `npm run test:ui`，自动将 L03 `Given-When-Then` 场景转化为 Headless 自动化点击。
...
### 第 8 课：Claude Code 开发，Codex 独立审查 — 上下文隔离与 CEO 3 大硬性决策树
* **定位**：**独立审查上下文隔离 (`AGENTS.md`)**，新建只读会话审计 Candidate Commit，注入《CEO 3 大硬性决策树》解锁高维裁决。
* **主要内容**：
  1. 遵循 [`AGENTS.md`](AGENTS.md) 规则，新建独立只读会话，消除开发 Agent 的历史思考污染。
  2. Codex 审查 Candidate Commit 的 Diff，仅输出自然语言审计意见（阻断/重要/建议），严禁修改代码。
  3. **双 Agent 仲裁与 CEO 3 大硬性决策树**：辩论超过 2 轮强行挂起，主管依据决策树执行裁决：
...
* **主要内容**：
  ...
  3. **Mock 降级保护桥梁 (Mock Fallback Bridge)**：真实 API 失败或超时时，前端 0.1 秒内自动降级渲染 L03 静态 Mock Data，确保演示 100% 不白屏崩盘。
