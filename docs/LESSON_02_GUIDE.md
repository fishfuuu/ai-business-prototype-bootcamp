...
2. **应用** 多模态（Multimodal）Agent 能力，从视觉参考图中仅提取布局结构与主次焦点，禁用参考图的杂乱背景色与字体。
3. **点开与查阅** `DESIGN.md` 中的设计字典，将组件样式映射至项目的 `--art-*` 设计变量。
...
```text
+-----------------------------------------------------------------------------------+
|                        第二课：视觉重构与设计规范 Harness                          |
+-----------------------------------------------------------------------------------+
| [输入]                [控制层 (Harness)]         [裁决与安全网]      [输出]         |
| 1. L1 系统原型 UI      1. DESIGN.md 字典          1. 节点 1 Commit   L2 高颜原型     |
| 2. 脱敏参考图 (多模态)   2. Agent 视觉结构提取      2. Keep/Modify 矩阵 (试衣镜 `http:// |
|                         - 仅提取布局主次           3. VS Code Diff    127.0.0.1:8888`|
|                         - 优先 --art-* Token       4. 脚本验证 PASS   渲染通过)       |
+-----------------------------------------------------------------------------------+
```
...
| :--- | :--- | :--- |
| **“美化页面就是把参考图的颜色抄过来”** | 绝不能抄参考图杂色！参考图只借布局主次，色彩由 `DESIGN.md` 的 Token 约定。 | 发送 Task 2 映射 Prompt，强制声明只能使用 `--art-*` 变量。 |
