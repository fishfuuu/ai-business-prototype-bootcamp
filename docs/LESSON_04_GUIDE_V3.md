...
### 1.2 底层运作原理
本课引入**受控 Agent 循环 (Controlled Agent Loop)** 与 **薄切片 (Thin Slices) 范式**：
- **先形成计划**：AI 在动手前必须先把复杂需求拆解为 3–5 个相互独立的薄切片，落盘为磁盘状态机 (`IMPLEMENTATION_PLAN.md`)。
- **状态机受控流转**：步骤状态按 `PENDING` ➔ `READY` ➔ `IN_PROGRESS` ➔ `COMPLETED` 流转，未经主管盖章授权，AI 物理上被禁止解锁下一个 Step 编码。
- **Working Tree 干净度控制**：通过 Git 的 `Working Tree (工作区)` 状态追踪，代码修改呈 Dirty，自测归档后物理恢复 Clean，失败则一键导出 `.patch` 快照并无损还原。
...
- **硬核工程定义**：Git 版本控制系统中，当前磁盘上允许被查看、修改和暂存的物理文件目录与工作镜像。
- **底层运作机制**：任何文件修改都会将 Working Tree 标记为 `Dirty` 状态；执行 `git commit` 或 `git restore` 后恢复为 `100% Clean` 干净状态。
...
- **底层运作机制**：在写入代码前先生成磁盘状态机（`IMPLEMENTATION_PLAN.md`），步骤状态按 `PENDING` ➔ `READY` ➔ `IN_PROGRESS` ➔ `COMPLETED` 受控流转。
...
- **硬核工程定义**：将前端界面的技术容错与加载状态（Loading/Empty/Error/Success）与数据对象的业务生命周期状态（待处理/已完成）进行物理隔离设计。
- **底层运作机制**：在组件内植入开发期专用的 `prototypeState` 调试开关，不依赖真实后端接口即可全量模拟各种极端网络交互。
- **具象业务比喻**：**新车试衣镜与极端风洞测试** 🚗。在不点火上路前，先静态测试车门、灯光、仪表盘在暴风雪中的容错表现。
