...
### ② Tools (Agent 交互工具)
* **硬核工程定义**：暴露给大模型的预定义 API 接口或系统函数（如 `read_file`、`write_file`、`run_command`），允许模型与外部文件系统及 CLI 终端建立受控交互。
* **底层运作机制**：模型在输出中生成特殊的 Function Call 语法结构，由本地 Agent 宿主程序拦截并实际执行该系统指令，再将执行结果作为新的上下文喂回模型。
* **具象业务比喻**：它们是 AI 的 **“双手与双脚”**。有了 Tools，AI 才能真正去翻看硬盘里的文件、敲击终端键盘。
* **IT 沟通与交接价值**：向 IT 说明：“ Agent 的操作严格限制在受控的命令行 Tools 沙箱内，杜绝任意代码执行风险”。

### ③ Agent (智能体)
* **硬核工程定义**：以大语言模型（LLM）为核心决策引擎，整合记忆组件（Context/Files）、感知接口（Prompts）与工具集（Tools），具备在特定目标驱动下进行自主多步循环推进的系统。
* **底层运作机制**：Agent 接收目标后，在 `感知 -> 思考 -> 工具调用 -> 结果观察 -> 下一步决策` 的闭环内持续运转，直到触发终止条件。
* **具象业务比喻**：它是拥有手脚和脑力的 **“高级执行程序员”**。给它明确的目标和手脚，它就能在授权范围内连续干活。
* **IT 沟通与交接价值**：向 IT 说明：“我们通过受控 Agent 完成原型增量代码编写，而非依赖无状态的单次 Prompt 盲猜”。

### ④ ReAct (Reasoning + Acting, 推理与行动闭环)
* **硬核工程定义**：一种将“思维链推理 (Thought)”与“工具行动 (Action)”交替交织的范式。Agent 每次行动后必须观察 (Observation) 工具的物理返回结果，并基于观察调整下一步推理。
* **底层运作机制**：`Thought (分析当前状态) -> Action (调用 Tool 修改代码或运行脚本) -> Observation (读取脚本报错或渲染结果) -> Thought (纠偏)`。
* **具象业务比喻**：这是 AI 研发中的 **“试错与试衣镜反馈”**。它不仅盲开代码，还会看着试衣镜里的报错和效果自动调优。
* **IT 沟通与交接价值**：向 IT 说明：“Agent 自带 ReAct 自测闭环，能在生成后自动运行单元测试脚本纠偏，确保代码零语法错误”。

### ⑤ Workflow 与 Human-in-the-Loop (流程框架与人在回路授权)
* **硬核工程定义**：定义 Agent 执行序列的节点图与硬性规则关卡，其中包含人工干预节点（Human-in-the-Loop, HITL），用于在关键状态转移前阻断 Agent 自动循环。
* **底层运作机制**：在 Agent 尝试执行写文件 (`write_file`) 前，系统拦截进程，强制等待人类用户在 CLI 终端输入许可信号（如 `同意方案`）。
* **具象业务比喻**：这是企业研发中的 **“CEO 盖章批准关卡”**。Agent 规划得再好，没有你盖章，一句话、一行代码都改不了。
* **IT 沟通与交接价值**：向 IT 说明：“原型制作全程受 HITL 门禁锁死，符合企业安全审计与合规控制要求”。
...
* **答案**：违反了 **Workflow 与 Human-in-the-Loop (人在回路授权门禁)**。救援方法：发送救援指令要求 Agent 停止修改，列出改动，并用 `git discard` 恢复。
