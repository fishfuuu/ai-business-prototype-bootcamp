...
- **核心突破口**：唤醒 `/grill-me` 互动追问，解构 6 大业务要素，冻结 3 份业务与数据契约，并通过 Task 3A 只读结构预览门禁实现受控写入。
...
2. **底层运作机制**：扫描工程上下文，针对使用者与问题、业务目标、边界范围、风险敏感度、`Given-When-Then` 与 `Stop / Escalation` 依次提问。
3. **具象业务比喻**：**装修开工前的主材确认清单** 📝。
4. **IT 沟通场景**：“需求已通过 `grill-me` 完成澄清，并落盘为包含 6 大要素的 `BUSINESS_FEATURE_CARD.md` 业务契约。”
...
| 08-25 分 | 极客示范 | **H & E** | 示范 Task 1➔3B，展示 `/grill-me` 与 Task 3A 只读预览。<br>**Pause Point 2**：“`/grill-me` 如何逼平 6 大业务要素？”<br>**Pause Point 3**：“三份契约包含哪些文件？” | 记录关键指令 |
| 25-35 分 | 概念核对 | **R** | 提问核对数据契约与三层模式对比图。<br>**Pause Point 4**：“Task 3A 只读预览门禁为何不可跳过？” | 口头回答卡片 |
