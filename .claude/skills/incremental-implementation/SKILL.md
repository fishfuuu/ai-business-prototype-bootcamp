...
### Phase 1: Read-Only Plan Preview & Pre-Plan Gate (权威交接门禁)

- **Authority Hierarchy**: `docs/BUSINESS_FEATURE_CARD.md` is the SINGLE AUTHORITATIVE SOURCE for requirements. `src/types/prototype-contract.d.ts` and `src/mocks/prototype-data.ts` are derived assets.
- **Pre-Plan Contract Consistency Gate**:
  1. All 3 files MUST exist.
  2. Derived TypeScript types and Mock data MUST strictly conform to the Data Contract table in `docs/BUSINESS_FEATURE_CARD.md`.
  3. If field names, types, requiredness, enums, or business semantics conflict:
     - Output `CONTRACT_ASSET_MISMATCH`.
     - List the conflicting fields.
     - **Do NOT generate or save the implementation plan**.
     - Instruct user to return to Lesson 03 to update the authoritative feature card or re-generate derived contract assets.
- If all 3 assets match, output proposed 3–5 step Contract-First execution plan preview in chat.
...
- **Step Status Enum**: `PENDING` (Initial future step) | `READY` (Awaiting authorization) | `IN_PROGRESS` (Authorized executing) | `COMPLETED` (Three-layer verification passed & committed) | `BLOCKED` (Execution, verification, or business acceptance rejected).
...
- **Step Constraints**:
  ...
  - Do NOT change the `allowed_files` list or `acceptance` criteria without explicit human authorization.
...
1. **Three-Layer Verification Order**:
   ...
   - **Layer 3: 主管业务验收 PASS** (Require explicit human authorization `主管验收 Step N 通过`).

2. **Outcome A: Verification & Human Approval PASS (三层全 PASS)**
   - Only when all 3 layers pass, present Two-Commit prompts and perform exact state transitions:
     - User inputs: `授权提交 Step N 源码`
     - Execute Commit A: `git add -- <Step N exact allowed_files>` -> `git commit -m "feat(prototype): step N - implement target slice"`
     - Obtain Commit A SHA (`git rev-parse HEAD`).
     - Update `docs/LESSON_04_IMPLEMENTATION_PLAN.md`:
       - `verification_log` -> `local-backups/lesson-04-evidence/step-N-verification.log`
       - `commit_sha` -> `<Commit A SHA>`
       - Step N `status` -> `COMPLETED`
       - Step N+1 `status` -> `READY` (if final step: `plan_status: COMPLETED`, `current_waiting_step: null`)
       - `current_waiting_step` -> `N+1` (if final step: `null`)
     - User inputs: `授权提交 Step N 状态推进`
     - Execute Commit B: `git add -- docs/LESSON_04_IMPLEMENTATION_PLAN.md` -> `git commit -m "docs(state): advance lesson 04 plan to step N+1"` (or `"docs(state): complete lesson 04 implementation plan"` for final step).

3. **Outcome B: Verification FAIL / Business Rejection / Step BLOCKED (失败工作区导出与自动恢复)**
   - Triggered when Verifier fails, manual page test fails, or human manager issues `主管拒绝 Step N 切片`:
     - **Do NOT execute Commit A** (do NOT commit broken or rejected code slice).
     - Stage untracked new files created during Step N: `git add -N -- <Step N exact allowed_files>`.
     - Export full patch containing exact allowed files: `git diff -- <Step N exact allowed_files> > local-backups/lesson-04-evidence/step-N-blocked.patch`.
     ...
     - **Clean Worktree Recovery**: Run `git restore -- <Step N exact allowed_files>` and remove untracked files within `Step N exact allowed_files` to restore 100% clean worktree!
     - Require independent HITL stamp: `授权提交 Step N 阻断状态` (DO NOT reuse state advance prompt!).
     - Execute Commit B ONLY for state recording: `git add -- docs/LESSON_04_IMPLEMENTATION_PLAN.md` -> `git commit -m "docs(state): record step N blocked status"`.
