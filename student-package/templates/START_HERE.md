...
- **不要**直接在 ZIP 压缩包内运行项目；请先完整解压。
- **不要**覆盖已有同名项目目录。
- 需要重新开始时，请解压到**新目录**。
- 每次重大修改前，可以复制整个项目目录作为本地备份。
- 示例备份目录名：

```text
AIPrototype-backup-before-lesson-01-change
```

- 本课**不要求**使用 Git 回滚；本地目录复制即可。

---

## 4. 安装与启动（约 5 分钟）

1. 将本 ZIP **完整解压**到推荐目录（或你选择的其他本地路径；尽量避免仅含特殊符号的深层路径）。
