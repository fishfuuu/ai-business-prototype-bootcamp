# 第九课学员指南 V4——画出部门 AI 机会与 Agent 范式选型图

> **本课核心思想只有一句话：**
> **不是"我的部门能不能用 Agent"，而是逐环节判断任务性质、可验证性、风险和合适范式。**

> **状态：V4_CONTENT_FROZEN（2026-08-12 用户确认）**。本课是正式的流程级业务 AI/Agent 机会判定课。从 L5 的"一个候选点"扩展为流程级主管判断。原版"确定性逻辑切割"概念不重复教授（L5 已覆盖）；原版 API MCP 和 Mock Fallback Bridge 删除（工程实现细节，超出主管所需深度）。
...
把今天的流程级选型想成城市交通规划：先画路网看每条路的客流需求（AI 机会地图），再逐条路选交通方式档次（AI 介入层级迁移），接着选具体交通工具（Agent 范式选型），然后区分三条供应链（三链区分），最后说明备降方案和责任（Fallback 来源与失败责任）。主管不是旁观者，而是逐环节判断、拒绝不该用 AI 的、选最简单足够范式的人。
...
- **类比与反例**：城市交通规划——不是先买公交车再找路线，而是先看每条路的客流需求，再决定哪条线适合公交、哪条适合地铁、哪条走路就行；不能每条路都配公交。
- **今天怎样看见**：Task 1—2 你会画出部门业务流程并逐环节评估，这就是 AI 机会地图的基础。
...
- **类比与反例**：工厂自动化程度迁移——L5 是在一个工序上试自动化，L9 是把所有工序排开逐个判断自动化程度；不是所有工序都配最高自动化，有些工序人工更好。
- **今天怎样看见**：Task 3 你会对每个环节选择 AI 介入层级，明确拒绝 AI/Agent 的环节及理由。
...
- **类比与反例**：工具箱选工具——锤子适合钉钉子，螺丝刀适合拧螺丝，不是每件工具都要用上，也不是越复杂的工具越好；选最简单足够的。
- **今天怎样看见**：Task 4 你会对保留候选选择范式，补数据、工具、HITL 和停止条件。
...
- **类比与反例**：建筑工地三条供应链——施工设备链（开发工具）、建材供应链（业务 API）、设计咨询链（模型 API）；三种链的供应商、合同和责任完全不同，不能混在一起管。
- **今天怎样看见**：Task 5 你会画三链责任与 fallback，明确每条链的 owner。
...
- **类比与反例**：航空公司备降——航班因天气备降其他机场时，必须告知旅客"备降原因、预计延误、替代方案"，不能假装"已到达目的地"；也不是"天气不好所以没人负责"。
- **今天怎样看见**：Task 5 你会画三链责任与 fallback，选出一个优先候选和一个明确不做项。
