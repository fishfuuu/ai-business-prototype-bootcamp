# 主管 AI 业务原型训练营：V4 十课冻结教学基线

> **状态：V4_CURRICULUM_CONTENT_FROZEN（2026-08-12）**
>
> 本文把已在对话中逐课确认的 L1—L10 教学内容写成一份课程级上游基线，供后续逐课生成 V4 教师教案、学员指南、核心概念关系图和核心词汇表使用。
>
> 本文不是十份完整教案，也不替代已经单独冻结的 L1 V4 教师教案和学员指南。它尚未写入正式仓库，不代表仓库交付生命周期已经转换。
...
5. Task 4 在试衣镜中验收，只修一个最影响理解或操作的问题，然后停止扩展并更新 `PROJECT_STATE.md`。
...
1. **信息优先级与视觉层级**：重要信息应先被看到，层级服务业务判断，不服务装饰。
2. **多模态参考提取**：从参考图提取结构、密度、颜色和交互意图，不是像素照抄。
3. **设计约束映射**：把参考意图写成可核对规则，并指出规则落在哪个页面元素。
4. **Keep / Omit / Modify**：主管对参考内容作保留、舍弃和改造裁决，不把决定交给 Agent 猜。
5. **视觉证据基线**：用前后对照和理由证明改变，不只说“更好看了”。
...
4. Agent 完成一次受控视觉重构；主管在试衣镜中做前后对比。
...
2. **结构化追问（grill-me）**：沿未决分支逐题澄清并得到主管确认，不是让 Agent 无限提问。
3. **业务契约**：把 User & Problem、Goal、Boundary、Risk、Acceptance、Stop/Escalation 固定为可执行约定。
4. **数据契约**：固定字段、含义、类型、枚举、来源状态和 Mock 规则，避免前后口径漂移。
5. **验收场景**：用给定条件、动作和可观察结果说明怎样算通过，不只列功能名称。
...
**可验证薄切片候选**：一个肉眼可见、可操作、可验收的变化，附契约追溯、状态说明和两阶段版本记录。

### 核心概念（4）

1. **薄切片**：一次完成一条从输入到结果的纵向变化，不是把任务拆成没有业务结果的技术碎片。
2. **Plan & Execute**：先形成经主管确认的实施计划，再按计划执行和调整；适合目标明确但步骤较多的任务。
3. **技术状态与业务状态**：加载、错误等系统状态和待处理、已升级等业务含义不能混为一谈。
4. **两阶段版本记录**：分别记录 Agent 完成的候选变化和主管的验收/处置，Git 本身不理解业务确权。

### 扩展认识（2）

- **HITL 授权点**：在范围、实施和验收处由人作决定；本课以动作实践，不扩成理论课。
- **回退与恢复**：恢复必须知道范围、证据和责任人，不把危险命令当课堂口令。
...
2. **事实锚定诊断**：从复现、输入、契约、日志和实际状态缩小原因，不先猜修复方案。
3. **五层诊断**：按环境/入口、数据、业务逻辑、界面交互、智能/外部依赖等层次定位问题，而不是到处乱改。
4. **有界排错**：明确一个故障、一个主假设、修改范围和轮次上限；失败也必须留下证据。
5. **停止、升级与恢复**：证据不足、风险越界或两轮无进展时停止，由指定人员接管或按范围恢复。

### 扩展认识（2）

- **重试预算/熔断**：次数、时间和风险达到阈值就不再自动继续。
- **根因与表象**：修掉页面报错不一定修掉业务或数据根因。
...
1. **MCP 最小模型**：Host 代表用户发起能力调用，Client 连接 Server，Server 暴露 Tools；MCP 不是产品业务 API。
2. **浏览器自动化验收**：Agent 按场景操作真实预览页面并观察结果，不是只读代码后自行宣布通过。
3. **验收场景迁移**：把 L3 的给定条件、动作和可见结果转成浏览器可执行步骤。
4. **证据链**：场景 → 操作 → 观察 → 原始证据 → 结论必须可追溯。
5. **验证、验收与责任**：工具验证行为，主管验收业务；测试通过不等于业务正确或可以上线。

### 扩展认识（2）

- **Playwright / DOM / Console / Network**：帮助 Agent 操作和取证的工程能力，只要求识别，不要求配置或背诵。
- **不可信输入与提示注入**：页面内容和工具返回可能夹带误导指令；Agent 不应把页面文字自动当作主管授权。
...
4. **Agent 协作模式选型**：根据依赖、并行性、独立性和责任选择流水线、调度/执行、并行、对等、评审等模式，不追求 Agent 数量。
5. **HITL 处置**：主管对 finding 作接受、修复、停止或延期决定，并记录承担理由。
...
1. **AI 机会地图**：以真实业务流程节点为单位判断，不以“部门想用 AI”为单位。
2. **AI 介入层级迁移**：不用 AI、辅助、建议、单步执行、受控 Agent 和受控闭环必须按风险与价值选择。
3. **Agent 范式选型**：在 Prompt、Skill、Workflow、ReAct、Plan & Execute、Routing、Evaluator–Optimizer、多 Agent 等方式中选择最简单足够者。
...
**产品决策与开发启动包**：包含问题与使用者、现有原型、业务/数据/验收契约、AI 机会与范式选型、测试和独立评审证据、风险红线、三链责任、未完成项、主管决策和五分钟业务汇报。
...
4. **开发启动包**：把已确认事实、证据、缺口和下一步交给 IT；不是一键生成的可上线包。
5. **责任交接**：业务、数据、模型、工具、审批、失败处置和上线门禁都有明确 owner。

### 扩展认识（2）

- **生产就绪缺口**：安全评审、真实数据治理、集成测试、监控、回滚、成本和合规等后续门禁。
- **能力分级**：可以演示、可以试点、可以有限内部使用、可以生产上线是不同等级。
...
| 上下文隔离—独立 finding—协作模式—主管处置 | L8 | L9 | L10 |
...
如果后续某课的 V4 教案或指南改变上述目标、主成果、核心判断或 AI 融入节点，应视为课程规格变化，先回到本基线讨论，不能由 Skill 自行改写。
