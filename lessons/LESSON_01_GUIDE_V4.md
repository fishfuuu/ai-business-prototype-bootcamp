# 第一课学员指南 V4（预览稿）——从业务问题到第一个受控原型

> **本课核心思想只有一句话：理解 LLM 大脑、Tools 手脚与 Agent 执行者的关系，并用这套协作关系建立第一个受控业务原型。**
>
> **状态：V4_CONTENT_FROZEN（2026-08-12 用户确认）**。这是后续 Skill 校准与 L2/L3 设计的课程内容基线，尚未写入正式仓库，不代表仓库生命周期已转换。A/B/C 是同一流程中的三个业务启动分支，不是三版指南。本课使用开发 Agent 制作普通业务原型；不判断业务 AI 机会，也不在原型中加入业务 AI 功能。
...
| 你得到的是一份候选回答 | 你在本地试衣镜中得到一条可操作的业务路径 |
...
→ Agent 实施 → 试衣镜观察 → 主管验收或只修一个问题 → 停止扩展
```
...
把今天的协作想成建造一个业务样板间：LLM 是总工程师，Tools 是施工机械，受控执行者（Agent）是组织施工的执行者，业务原型是供主管验收的样板间。主管不是旁观者，而是决定建什么、建到哪里、结果是否合格的人。

#### 1. 大语言模型（LLM）：大脑

- **是什么 / 不是什么**：根据当前信息生成文字、方案或代码内容的概率模型；不是能直接操作电脑的完整 Agent，也不是天然了解部门事实的业务专家。
- **怎样运作**：根据 Prompt 与当前上下文生成候选内容；内容听起来合理，也可能遗漏或猜错事实。
- **类比与反例**：总工程师可以理解要求、画图纸；“图纸画好了”不等于“样板间建好了”。
- **今天怎样看见**：Task 1 只让它复述和规划。主管应核对它是否真正理解了业务。

#### 2. Tools：手脚

- **是什么 / 不是什么**：读取文件、修改项目、运行命令或取得结果的具体能力；不是 LLM 本身，也不代表权限无限或绝对安全。
- **怎样运作**：Agent 请求调用工具，宿主环境实际执行，再把读取结果、运行输出或错误返回给模型。
- **类比与反例**：施工机械和测量工具让方案能够落地；“有机械”不等于施工天然安全合规。
- **今天怎样看见**：Task 2 看真实读取结果，Task 3 看实际修改与运行。主管应问清读了什么、改了什么。

#### 3. 受控执行者（Agent）

- **是什么 / 不是什么**：把 LLM、当前信息和 Tools 组织起来，为一个目标完成多步工作的系统；不是永远正确、能够自行承担业务责任的数字员工。
- **怎样运作**：接收目标，推演下一步，调用工具，观察结果，再继续、调整、停止或请求人工处理。
- **类比与反例**：现场执行负责人能够看图纸、调度机械；不能在没有业务边界和验收时自行决定建什么。
- **今天怎样看见**：Task 1—4 组成一次完整制作过程。开发 Agent 负责执行，主管负责目标、范围和验收。

#### 4. 业务原型：样板间

- **是什么 / 不是什么**：用来验证业务路径、交互和需求表达的可操作模型；不是生产系统，也不代表全部需求已经完成。
- **怎样运作**：用 Mock/已脱敏数据呈现一个使用者如何完成一件事，再由主管判断路径是否正确。
- **类比与反例**：样板间可以验证关键布局；不能因为样板间能参观，就说整栋建筑已经交付。
- **今天怎样看见**：你最终完成的一条个人业务路径，并能够说明哪些已经验证、哪些仍未实现。

**Prompt** 是主管表达任务的工具，不作为第五个核心概念。**ReAct** 是“执行一个动作—观察结果—根据结果调整”的现象，本课只要求能认出来；它不保证 Agent 一定能自动修好问题。

## 二、准备好：选择一个原型起点并打开试衣镜
...
打开本地试衣镜：双击项目根目录的 `start-project.bat`；如教师要求使用终端，可运行 `npm run dev`。浏览器未自动打开时访问 `http://127.0.0.1:8888/`。超过 3 分钟仍打不开，请助教接管。页面能显示只说明环境可用，不代表业务正确。
...
一句话记忆：**主管用 Prompt 说清任务，LLM 负责理解和推演，Tools 负责读取、修改与运行，Agent 把动作组织起来；主管最后通过试衣镜验收业务原型。**
