# 第八课学员指南——Claude 执行，Codex 独立评审，主管裁决
...
今天直接沿五个 Task 看见五个概念：Task 1 汇总输入，Task 2 建立隔离上下文，Task 3 让 Codex 输出带引证的 finding，Task 4 由主管逐项处置，Task 5 对 fix 项修复并重新取证。主管不是旁观者，而是逐项处置 findings 并承担批准责任的人。

#### 1. 上下文隔离

- **是什么 / 不是什么**：上下文隔离（双盲独立评审）：评审模型只看最终代码与运行证据，坚决不给它看开发过程中的聊天记录。就像“盲审”一样，防止 AI 自己给自己辩解、掩盖缺陷。
- **怎样运作**：评审者拿到的是一份"隔离输入包"——契约、候选变更和证据索引，而不是开发过程中的全部对话和推演；评审者不知道实现者怎么想的，只看结果和证据。
- **当课操作例子与边界**：Task 2 只给评审者契约、候选变更和证据索引，不传开发对话和自我辩护；材料超出约定范围时先删减。
...
- **当课操作例子与边界**：Task 3 的每条 finding 都写清问题、业务影响、证据引证和建议，并标注优先级；没有引证的判断不能直接进入 Task 4。

#### 3. 验证、评审与批准三分

- **是什么 / 不是什么**：执行者可验证（自己检查行为是否正常），独立者评审（第三方看证据找问题），主管批准或拒绝；三者不能压成一个自动通过点。自验、评审与拍板三权分立：执行者负责自验（自己做对了吗），独立模型负责挑刺（第三方看有没有问题），业务主管负责拍板（决定接不接受现状）。三者各司其职，绝不能由 AI 一键自评自过。
- **怎样运作**：Claude 执行并自验行为，Codex 独立评审找问题，主管读取 findings 后批准、打回或延期；三者由不同角色承担，不能由同一个流程自动完成。
- **当课操作例子与边界**：Task 4—5 中，Claude 执行和自验，Codex 独立评审，主管读取 findings 后批准、打回或延期；任何一步都不能冒充另一种职责。
- **帮助理解的类比（可选）**：像工程建设的三方制衡：施工队砌完墙自己拉线靠尺（执行者验证），监理公司对照规范出质检报告（独立评审），业主拿合同决定是否签字收楼（主管批准）。施工队不能自宣布合规，监理也不能替业主签字。

#### 4. 独立评审

- **是什么 / 不是什么**：由未参与实现的评审者在隔离上下文中检查规格、证据和变更；不是协作模式选型，也不是实现者自评或主管批准的替代品。
- **怎样运作**：Task 1—2 只在需要独立判断时选择评审模式；评审者接收必要输入，主管再作处置，选择最简单足够的协作方式。
- **当课操作例子与边界**：Task 1—2 汇总评审输入并建立隔离评审时，已经实践“执行者和评审者分离”；第七课的 Subagent 调度属于另一种操作方法，不能替代本课独立评审。
...
- **当课操作例子与边界**：Task 4—5 逐项读取 finding、证据和业务影响，决定 accept/fix/stop/defer 并写理由；评审者不能替主管作出决定。

**Agent 协作模式选型**与 **accept/fix/stop/defer** 是操作方法；六种协作模式名称和 Reviewer fallback 只作背景，不占核心概念名额。
...
打开本地预览确认原型仍可操作。超过 3 分钟仍打不开，请助教接管。
