...
- **是什么 / 不是什么**：AI 介入是有梯度的（不用 AI、辅助建议、固定流程、受控决策）；不是非黑即白，更不是越自主、越复杂就越好。
...
- **怎样运作**：Workflow 适合步骤固定、路径明确的任务；Agent 适合需要根据中间结果调整路线的任务；判定黄金法则：凡是步骤固定、规则明确的，坚决走固定流程（Workflow）；只有需要根据中间结果动态调整下一步的，才考虑 Agent。
