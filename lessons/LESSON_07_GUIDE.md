...
- **是什么 / 不是什么**：MCP 协议（标准化工具接口）：把 AI 想象成电脑主机（Host），MCP 是标准的 USB 连接层，外接的浏览器或测试服务就是打印机（Server）。它只负责连接工具，不负责替你做业务判断。Host 是用户运行的主程序（如 Claude），Client 是连接层，Server 是工具提供方（如 Playwright）。
- **怎样运作**：主管在 Host 中发起请求，Client 把请求传给 Server，Server 执行工具（如打开浏览器、点击按钮、截图）并返回结果；MCP 只是把"工具调用"标准化，不改变谁来负责。
- **当课操作例子与边界**：Task 2 的浏览器验收中，Host 发起能力调用，Client 连接 Server，Server 暴露并执行 Tool；MCP 只说明工具怎么调用，不判断业务是否通过。
...
- **是什么 / 不是什么**：Subagent 是主管调度、限定任务和上下文的子 Agent；不天然等于只读、隔离或安全。课程的只读、超时、授权和落盘要求属于操作规程。
...
- **当课操作例子与边界**：Task 2—3 调度 Subagent 用浏览器操作页面并保存截图、日志和报告；只读、超时、授权和落盘是本课规程，不是 Subagent 名称自带的能力。
- **帮助理解的类比（可选）**：像项目主管派出的"现场跑腿调研员"：只领一张明确任务清单（只读指令），带相机和笔记本去现场拍照取证并带回汇报；他没有签字权（无写权限），遇到计划外变动必须回办公室请示，无权私自替主管做决定。

#### 3. 自动化验证与业务验收

- **是什么 / 不是什么**：自动化验证检查可观察技术事实，业务验收由主管判断目标是否满足；两者都不能替代对方。
- **怎样运作**：取 L3/L4 的验收场景，保持业务含义不变，把"给定条件"翻译为页面初始状态，把"动作"翻译为浏览器操作步骤，把"可见结果"翻译为可观察的页面表现。
- **当课操作例子与边界**：Task 1 保持 L3/L4 验收场景的业务含义不变，只把给定条件、动作和结果翻译成浏览器步骤；不能借迁移之机改需求。
...
- **当课操作例子与边界**：Task 3—4 每一步都记录场景、操作、观察、原始截图或日志和结论；缺少任一环节就标为未证明，不用结论替代原始证据。

#### 5. 不可信输入与提示注入

- **是什么 / 不是什么**：外部页面、消息和数据可能夹带误导指令，不能自动当作主管授权；不是普通输入或业务 Bug 的同义词。
- **怎样运作**：把外部文本当作待验证数据，回到约束和授权核对，拒绝越权动作；Agent 可以收集证据但不能批准，主管基于证据批准或打回。
- **当课操作例子与边界**：Task 5 先说明工具验证了什么，再由主管对照业务验收标准判断是否通过；页面文字要求执行额外动作时，停止并上报，不把它当授权。
