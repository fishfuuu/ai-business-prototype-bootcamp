...
把今天的薄切片实施想成装修施工：先确认哪面墙可以单独验收并写清本轮做到哪里（薄切片），再按计划确认每步改哪里，接着区分门能不能开和车有没有上牌（技术状态与业务状态），最后用版本证据追溯验收。主管不是旁观者，而是决定做哪一块、做到哪里、验不验收的人。
...
- **是什么 / 不是什么**：识别 Working Tree、Diff、Commit 的版本证据，并分别记录候选变化和主管验收/处置；两阶段版本记录只是本课程方法，不要求两个 Commit。
- **怎样运作**：记录 Working Tree、Diff 或 Commit 的实际变化，并把候选实现与主管处置关联；提交次数由项目需要决定，不以两个 Commit 作为通过条件。
- **类比与反例**：买房交房和过户可以帮助理解“候选实现”和“业务处置”两种记录；候选实现与业务处置应分别留痕，但提交数量不固定。
