...
今天不额外引入一套故事，直接沿着五个 Task 看见四个概念：Task 1 固定业务问题边界，Task 2 用结构化追问补齐信息，Task 3 收敛业务契约，Task 4 固定数据契约，Task 5 写出验收场景并由主管确认。概念是否成立，以对应 Task 的产物和确认结果为准。

#### 1. 业务问题边界

- **是什么 / 不是什么**：使用者、触发、问题、目标和明确非目标共同限定本次业务；不是一句话需求，也不是全部部门需求。
- **怎样运作**：回答谁遇到什么问题、为什么要做、目标是什么、边界在哪里；缺少任何一项都会让后续实施失去锚点。
- **当课操作例子与边界**：Task 1 只选一条业务路径，Task 2 前两轮追问补齐使用者、问题、目标和范围；如果一条路径包含多个主要目标，先拆开，不把全部部门需求算进本课范围。

#### 2. 业务契约

- **是什么 / 不是什么**：把业务目标、角色、流程、规则、边界和通过条件写成可核对的约定；不是口头共识，也不是大而全 PRD。
- **怎样运作**：Task 2 的确认结果在 Task 3 收敛为业务功能卡；未确认项显式保留，冻结后只能在约定范围内选择薄片。
- **当课操作例子与边界**：Task 3 的业务功能卡是本课业务契约的落点；没有主管确认的内容仍是待确认项，不能直接进入第四课实施。

#### 3. 数据契约

- **是什么 / 不是什么**：固定字段、类型、枚举、来源和缺失行为等数据结构约定；不是 TypeScript 工程任务，也不是业务目标本身。
- **怎样运作**：为每条业务路径列出字段字典，标注含义、类型、是否必填、Mock 样例和敏感等级；工程师可据此写类型或 Schema。
- **当课操作例子与边界**：Task 4 的字段字典和 Mock 样例必须与 Task 3 的业务功能卡一致；字段名、类型或敏感等级不一致时先回到契约修正。
- **帮助理解的类比（可选）**：像海关进口的标准报关单：品名、商品编码、计量单位、申报单价必须逐字统一，不能前端叫"订单号"后端叫"工单号"，数量栏也不能填"少量"。任何字段对不上，货物（数据流）就在关口被拦下。

#### 4. 验收场景

- **是什么 / 不是什么**：包含前置条件、操作、预期观察、通过标准和失败处置的可重复业务情境；不是功能名称列表，也不是主观感觉。
- **怎样运作**：把给定条件、动作和可观察结果写清，遇到数据缺失或规则冲突时触发停止并升级。
- **当课操作例子与边界**：Task 5 的 1—2 条场景必须能回指业务功能卡和数据契约；没有可观察结果的句子不能作为验收场景。

#### 5. Skill

- **是什么 / 不是什么**：Skill 是一份可复用的打包指令（`SKILL.md`），Agent 装载后按它执行某种固定方法；不是一次性 Prompt，也不是 Agent 本身或某个工具。
- **怎样运作**：需要某类方法时激活对应 Skill，Skill 把方法步骤固化、可跨项目重用；Agent 按步骤执行并在边界内停，主管仍核对结果。
- **当课操作例子与边界**：Task 2 激活 `grill-me` Skill 做逐题追问；Skill 固定的是“怎么问”，不替代主管对业务答案的确认；第四课的 `incremental-implementation` 是“先出计划再执行”的 Skill。
- **帮助理解的类比（可选）**：像岗位 SOP 和临时口头交代的区别——SOP 把一套做法写成手册、人人按同一套执行，口头交代是一次性的；`grill-me` 就是“逐题追问”的 SOP。

#### 操作方法：结构化追问（Grill-Me）

- **是什么 / 不是什么**：按未决分支逐题澄清并获得主管确认的操作方法；不是让 Agent 无限提问，也不是一次性抛出所有问题。
- **怎样运作**：每次只问一个最关键的问题，主管确认后再进入下一题，覆盖问题、目标、边界、风险、通过和停止/升级条件。
- **当课操作例子与边界**：Task 2 激活 grill-me 后，只有当前问题得到回答和确认，才进入下一轮；验收场景写作属于 Task 5。

**敏感度分级与事前脱敏** 是扩展认识：数据进入对话和材料前先判断敏感等级（公开/内部/严禁发送 AI），不在泄露后再补救。**结构化输出/类型文件** 也是扩展认识：工程师可把数据契约写成 TypeScript 类型或 Schema；主管只需核对业务含义，不要求自己写类型文件。
...
npm run dev，打开本地预览确认第二课原型仍可操作（http://127.0.0.1:8888/）。页面能显示只说明环境可用，不代表业务正确。超过 3 分钟仍打不开，请助教接管。
...
1. 拟定 Markdown 文档《业务功能卡》内容预览（共 9 大完整章节），包含：
   - User & Problem（使用者与问题）
   - Goal（业务目标）
   - Boundary（本轮做什么、不做什么）
   - Risk（数据敏感度与错误代价）
...
6. Skill 是可复用的打包指令（`SKILL.md`），把一套方法固化、跨项目重用；一次性 Prompt 是临时写的单次指令。grill-me 把“逐题追问”这套方法固化下来重复使用，所以是 Skill。
