...
**重试预算/熔断** 是扩展认识：次数、时间和风险达到阈值就不再自动继续。原版的"2 轮硬熔断"是具体实现：两轮不通则强制停止。**根因与表象** 也是扩展认识：修掉页面报错不一定修掉业务或数据根因。两者只要求听过、能识别，不设独立通过门槛。
...
打开试衣镜确认第五课原型仍可操作。超过 3 分钟仍打不开，请助教接管。
...
**本步目的**：修复后实际刷新试衣镜看效果。$diagnose 的 Phase 5 要求写回归测试，但在本课中回归测试由 Agent 自动完成，你只需确认原始故障不再复现。通过则进入 Task 5 记录；不通过则调整假设进行第二轮；两轮都不通则停止。
...
| "AI 说已修复就是修好了" | 必须实际刷新试衣镜看效果，不看 AI 自称结果 |
...
| **AI 自称"已修复"但无实际效果** | 要求实际刷新试衣镜验证，不看 AI 自称结果 |
