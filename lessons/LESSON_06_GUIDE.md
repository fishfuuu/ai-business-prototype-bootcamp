...
- **当课操作例子与边界**：Task 1—2 的事实卡只记录页面、数据和日志中实际看到的内容；诊断报告中的原因仍标为假设，Agent 自称“已修复”但没有证据时要当作生成错误。
...
- **当课操作例子与边界**：Task 1—3 先复现、记录事实、审查五层定位报告，再审查修复方案；没有能证伪的主假设和证据，不授权修改。
- **今天怎样看见**：Task 1—3 你会先审查 Agent 的事实卡，再审查五层定位报告，再审查 Agent 的修复方案后才授权；诊断结论落盘到 `LESSON_06_DEBUG_REPORT.md`。

#### 3. 重试预算/熔断

- **是什么 / 不是什么**：为自动重试设置次数、时间或风险上限，达到阈值就停止；不是固定数字，也不是定位问题的五层标签。
- **怎样运作**：每轮记录动作、观察和成本，达到预算后停止、升级或恢复。本课为保证证据和安全，Agent 诊断/修复最多进行两轮；这是本课规则，不是所有系统的通用数字。
- **当课操作例子与边界**：Task 2 先用五层诊断定位主假设，Task 3—4 记录每轮实际修改和验证结果；超过两轮或触发风险条件就进入 Task 5。
- **今天怎样看见**：Task 2 你会审查五层定位报告，Task 3—4 会看到轮次、观察和成本，Task 5 记录熔断后的处置。
...
- **当课操作例子与边界**：Task 3—4 先保存 Checkpoint，再按主假设修改；第一轮验证不通过才调整假设做第二轮，仍不通过就停止，不继续扩大范围。
...
- **当课操作例子与边界**：Task 5 记录停止原因、新证据需求和恢复责任人；恢复只能按确认范围进行，不能用无差别清空代替交接。
