...
今天直接沿 Task 1—5 看见五个概念：Task 1 提取参考意图，Task 2 写入并核对项目约束，Task 3 做 Keep/Omit/Modify 裁决，Task 4—5 用前后证据和业务路径验证一致性。多模态提取、设计映射、Keep/Omit/Modify 和前后证据是完成任务的方法与产物。

#### 1. 项目级行为护栏（Agent 约束规范）

- **是什么 / 不是什么**：项目给 AI 提前立好的规矩与底线，规定它能动什么、不能动什么、界面必须长什么样；不是单次 Prompt 或运行时硬控制。
- **怎样运作**：Agent 读取项目约束，主管核对关键红线和设计规则，再在授权范围内执行。
- **当课操作例子与边界**：Task 2 检查并使用项目约束文件；约束能指导 Agent，但不等于运行时自动拦截，授权和验收仍由主管负责。
- **帮助理解的类比（可选）**：像给外协团队的项目章程：写明目标、范围和红线，但不替他们每步盯办；授权和验收仍由主管。章程不等于运行时自动拦截。

#### 2. 约束作用域与优先级

- **是什么 / 不是什么**：规则按“项目级 > 目录级 > 单文件级”逐级生效。遇有冲突时，全局红线高于局部规则，任何单个文件都不能擅自突破项目底线。
- **怎样运作**：先确认当前文件和任务落在哪个作用域，再按优先级处理冲突。
- **当课操作例子与边界**：Task 2 先判断修改落在哪个项目、目录或文件，再按上级规则处理冲突；局部文件不能覆盖项目红线。
- **帮助理解的类比（可选）**：像公司规章、部门细则、单据模板的层级：局部细则不能覆盖公司红线。不是所有规则同层。

#### 3. 行为约束与设计约束

- **是什么 / 不是什么**：行为约束 = `CLAUDE.md`/`AGENTS.md`，规定 Agent 怎么工作（包管理器、文件边界、错误处理）；设计约束 = `DESIGN.md`，规定产物长什么样（间距、圆角、颜色层级）；两者都是项目级约定，不是代码级强制。
- **怎样运作**：Agent 生成前必须读取这两类约束：设计约束用参考图映射后由主管核对，行为约束用 `/init` 生成基础版后由主管核对；规则必须指向具体页面元素或具体行为红线。 行为约束文件按工具分：Claude Code 读 `CLAUDE.md`、Codex 读 `AGENTS.md`；两个工具都用时，两边都要配置行为红线（或在 `CLAUDE.md` 里用 `@AGENTS.md` 互引做单一真相源），否则未配置的一方会无视规则越界。
- **当课操作例子与边界**：Task 2 分别核对 DESIGN.md 的页面元素约束和 CLAUDE.md/AGENTS.md 的行为红线；一类文件不能替代另一类。

#### 4. 信息优先级与视觉层级

- **是什么 / 不是什么**：按业务重要性安排内容、状态和操作的视觉强调；不是装饰偏好。
- **怎样运作**：逐项审视参考图中的每个元素，判断"保留并冻结现有业务内容""参考图忽略项""现有页面清理项"和"明确进行的视觉改进"。
- **当课操作例子与边界**：Task 3 对每个参考意图做 Keep/Omit/Modify 裁决并写业务理由；参考图不能带入自己的业务内容。

#### 5. 约束一致性验证

- **是什么 / 不是什么**：检查实现是否同时满足项目约束、设计约束和验收场景，并留下证据；不是只看文件存在。
- **怎样运作**：重构前截图作为基线，重构后截图作为对照，每处改变附理由；业务路径在视觉修改后仍可操作。
- **当课操作例子与边界**：Task 4—5 对照重构前后截图、约束清单和业务路径；视觉变化没有理由，或页面路径不可操作，都标为未通过。

**设计令牌（Design Token）** 把颜色、间距、字号等重复规则统一命名；教师一句话说明，不作为本课工程重点。**行为约束文件 CLAUDE.md/AGENTS.md** 是扩展认识：Agent 在修改代码前读取行为红线，主管只核对包管理器、文件边界和错误处理是否覆盖。**版本节点** 保留批准前后候选差异的记录；不要求你掌握 Git 命令，但每次大改前先存档，改崩后按确认范围恢复。

## 二、准备好：选择参考图并打开本地预览

### 选择参考图

使用你在第一课课后准备的那张业务相关视觉参考图；若没有图片，可用下面的「文字版参考图卡」代替：

- **监控与决策型**：顶部 KPI 指标栏 + 中部图表区（趋势/占比）+ 底部明细表；主视觉焦点在关键指标。
- **任务与流程型**：顶部筛选与统计 → 中部业务列表/看板（含状态列、优先级）→ 行内操作按钮；焦点在“待办”列表。
- **操作工具型**：输入/表单区 + 校验提示 + 确认按钮 + 结果预览；焦点在核心操作区。

参考图用来提取布局意图，不是照搬其中的业务字段和数据。

### 打开本地预览

1. 打开 VS Code 左侧文件树，双击点开根目录下的 `DESIGN.md`，确认包含小写 CSS Tokens（如 `var(--art-primary)`）。
2. 打开本地预览：双击项目根目录的 `start-project.bat`；如教师要求使用终端，可运行 `npm run dev`。浏览器未自动打开时访问 `http://127.0.0.1:8888/`。
   ```powershell
   npm run dev
   ```
...
```text
请基于我第一课的视觉参考图分析视觉结构；若没有图片，请按以下文字版参考图卡分析（监控决策型=顶部 KPI 栏+中部图表+底部明细表；任务流程型=顶部筛选+中部列表/看板含状态列+行内操作；操作工具型=输入/表单+校验+确认+结果预览）：
...
**本步目的**：Agent 在授权范围内执行一次受控视觉重构，你在本地预览中做前后对比。
...
1. 刷新 `http://127.0.0.1:8888/` 本地预览，对比重构前后效果。
...
核心路径通过后，让 Agent 更新 `docs/PROJECT_STATE.md`，记录本次设计约束、Keep/Omit/Modify 裁决和未实现项。它是下一课的项目交接单，不是第二份主要作业。
...
1. 在 `PROJECT_STATE.md` 中补齐本次设计约束和 Keep/Omit/Modify 裁决记录；
