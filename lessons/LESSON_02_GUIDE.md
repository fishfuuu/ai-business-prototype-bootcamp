...
把今天的视觉重构想成装修一套二手房：先确认项目级 Agent 约束，再核对规则作用域与优先级，区分行为约束和设计约束，按信息层级安排视觉重点，最后用一致性验证证明实现同时满足这些约束。多模态提取、设计映射、Keep/Omit/Modify 和前后证据是完成任务的方法与产物。
...
- **类比与反例**：施工总规约；它能指导施工，但不会像代码拦截器一样自动阻止每个错误动作。
...
- **类比与反例**：施工现场的总规约优先于某个房间的便签；局部便签不能覆盖安全红线。
