...
#### 1. 项目级 Agent 约束 / Agent Harness

- **是什么 / 不是什么**：项目为 Agent 规定目标、范围、行为和设计规则的约束机制；不是单次 Prompt 或运行时硬控制。
...
- **是什么 / 不是什么**：规则按 项目级、目录级、文件级 分层适用，冲突时上一级压下一级；局部文件不能覆盖项目红线，也不是所有规则同层。
...
- **怎样运作**：Agent 生成前必须读取这两类约束：设计约束用参考图映射后由主管核对，行为约束用 `/init` 生成基础版后由主管核对；规则必须指向具体页面元素或具体行为红线。 行为约束文件按工具分：Claude Code 读 `CLAUDE.md`、Codex 读 `AGENTS.md`；两个工具都用时，行为红线两边都要有（或在 `CLAUDE.md` 里用 `@AGENTS.md` 互引做单一真相源），否则一边静默看不到。
