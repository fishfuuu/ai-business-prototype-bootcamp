...
- **今天怎样看见**：Task 2 检查项目约束文件并将其用于视觉重构。

#### 2. 约束作用域与优先级

- **是什么 / 不是什么**：约束适用的项目、目录、文件或任务范围，以及冲突时采用的上级规则；不是所有规则同层。
- **怎样运作**：先确认当前文件和任务落在哪个作用域，再按优先级处理冲突。
- **当课操作例子与边界**：Task 2 先判断修改落在哪个项目、目录或文件，再按上级规则处理冲突；局部文件不能覆盖项目红线。
- **今天怎样看见**：Task 2 标出行为约束和设计约束分别适用的范围。

#### 3. 行为约束与设计约束

- **是什么 / 不是什么**：行为约束规定 Agent 如何工作，设计约束规定产物的视觉和交互要求；不是代码级强制。
...
- **今天怎样看见**：Task 4—5 你会在本地预览中做前后对比，确认业务路径仍可操作，并保存视觉证据。
