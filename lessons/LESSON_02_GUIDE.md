...
- **是什么**：放在项目根目录的“AI 员工入职守则（物理规则文件）”，任何 AI 工具都会自动读取它。
- **怎么用**：在 CLAUDE.md 或 AGENTS.md 中写明项目结构、受保护目录、设计规范及自检流程，实现规则全局常驻。
- **避坑红线**：警惕注意力漂移！规则文件不是硬编译器，会话过长时大模型会遗忘规则；任务完成后及时开新会话，主管必须亲自对照护栏核对 Diff。
