...
- 首次核心概念使用四步概念解析公式：是什么/不是什么、怎样运作、当课操作例子与边界、刚完成的实操与主管价值；定义已经能讲清时不强行添加类比，确需类比时保持一套稳定映射。
...
1. **业务问题边界**：使用者、触发、问题、目标和明确非目标共同限定本次业务范围。
2. **业务契约**：把业务目标、涉及角色、处理规则与验收标准白纸黑字写成确定性协议，不让 AI 凭空瞎猜。
3. **数据契约**：固定字段、类型、枚举、来源和缺失行为等数据结构约定。
4. **验收场景**：包含前置条件、操作、预期观察、通过标准和失败处置的可重复业务情境。
5. **Skill**：可复用的打包指令（SKILL.md），Agent 装载后按它执行某种固定方法；区别于一次性 Prompt。
...
2. **版本证据与回退**：识别 Working Tree、Diff、Commit 如何提供版本证据，不规定提交次数。
...
3. **产品决策**：基于业务价值、证据、风险和成本，并明确责任，选择继续、调整、暂缓或停止，不以“做出来了”自动等于应该建设。
4. **生产就绪缺口**：原型到生产之间仍需补齐的安全、真实数据、集成、测试、监控、回滚、成本和合规事项。
