...
1. **引言顶置核心金句与【背景/原理/目标】专栏**：
   - 顶部用 `> 💡 **本课的核心思想只有一句话：...**` 顶置理念。
   - 紧接着输出 `## 一、 本课背景、底层原理与学习目标`（包含业务背景与痛点、底层运作原理、核心学习目标）。
   - **禁止输出无意义的元说明废话**（如“本指南支持双模式使用”等）。
2. **集中【💡 本课核心工程概念卡】专栏**：
...
4. **Task 任务双轨道设计**：
   - 每个 Task 必须区分：`⚡ 极速操作步骤`、`💡 独立自学原理解析` 与 `🔍 代码 Before/After 对比`。
5. **双重误区与排错保障**：
   - 必须同时包含 **`💡 常见概念误区与正确理解` (思维观念重塑 4 大误区表)** 与 **`❓ 常见操作报错` (Troubleshooting 排错指南)**。
6. **灵活测试题库 (3–5 题)**：
   - 题库数量不写死为 2 题，而是根据当堂课重点教学内容**灵活设计 3–5 道题（上限 5 题）**，拆分为“阶段 1 课堂退场测试”与“阶段 2 课后自学拓展思考题”。
...
- [ ] 学员指南引言包含核心金句与【背景、原理、目标】专栏（无元说明废话）。
